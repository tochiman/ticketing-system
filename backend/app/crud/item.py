from sqlalchemy.sql import select, delete, case
...
async def get_item_with_availability(db, organization_id, store_id):
    available_exists = (
        select(case((models.Available.item_id != None, True), else_=False))
        .where(models.Available.item_id == models.Item.item_id, models.Available.store_id == store_id)
        .limit(1)
        .scalar_subquery()
    )
    stmt = (select(models.Item, available_exists.label('is_available')).where(models.Item.organization_id == organization_id, models.Item.disabled == False))
    results = (await db.execute(stmt)).all()
    items = []
    for item, is_available in results:
        item_dict = {
            "name": item.name,
            "size": item.size,
...
async def delete_item(db, item_id):
    stmt = delete(models.Available).where(models.Available.item_id == item_id)
    await db.execute(stmt)
    stmt = select(models.Item).where(models.Item.item_id == item_id, models.Item.disabled == False)
    item = (await db.execute(stmt)).scalars().unique().first()
    item.disabled = True
